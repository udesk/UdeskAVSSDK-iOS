...
- (void)setupViews{
    ...
    
    {
        ...
        UIView *localView = [[UIView alloc] initWithFrame:CGRectMake(kScreenWidth - 106, 64, 90, height)];
...
{
    if ([UdeskAVSSDKManager sharedInstance].customerBackgroundUrl.isNotEmpty) {
        NSLog(@"context=%@", [UdeskAVSSDKManager sharedInstance].customerBackgroundUrl);
    }
    if (self.context.isChatShowing.boolValue) {
        self.context.isChatShowing = @(0);
        return;
    }
