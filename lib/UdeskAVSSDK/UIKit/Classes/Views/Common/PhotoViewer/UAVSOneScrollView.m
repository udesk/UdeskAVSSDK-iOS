...
@interface UAVSOneScrollView()<UIScrollViewDelegate>
{
...
@end
@implementation UAVSOneScrollView
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        //添加主图片显示View
        UIImageView *mainImageView = [[UAVS_YYAnimatedImageView alloc]init];
...
-(void)setFrameAndZoom:(UIImageView *)imageView withMessageURL:(NSString *)url
{
    ...
    if(imageView.image == nil || imageView.image.size.width == 0 || imageView.image.size.height ==0)
    {
        ...
        self.mainImageView.image = [UIImage udavs_imageNamed:@"udImageLoading"];
        
        if (url && url != (id)kCFNull && url.length > 0) {
            [self.mainImageView uavs_yy_setImageWithURL:[NSURL URLWithString:[url stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]] placeholder:imageView.image];
        }
    }
    else {
        //设置主图片
        ...
        [self.mainImageView uavs_yy_setImageWithURL:[NSURL URLWithString:[url stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]] placeholder:imageView.image options:UAVS_YYWebImageOptionShowNetworkActivity completion:^(UIImage * _Nullable image, NSURL * _Nonnull url, UAVS_YYWebImageFromType from, UAVS_YYWebImageStage stage, NSError * _Nullable error) {
            
            if (!image) {
                return ;
            }
            UIImage *newImage = [UdeskAVSImageUtil fixOrientation:image];
