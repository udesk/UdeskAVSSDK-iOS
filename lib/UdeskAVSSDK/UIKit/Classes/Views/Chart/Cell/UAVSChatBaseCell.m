...
@implementation UAVSChatBaseCell
...
- (YYLabel *)msgStatusLabel {
    if (!_msgStatusLabel) {
        ...
        _msgStatusLabel.font = [UIFont systemFontOfSize:kUAVSChatMessageStatusFontSize];
...
//    else
    if (baseLayout.message.agentAvtarImage) {
        ...
        NSString *imageUrl = [UdeskAVSUtil urlStringEscapedEncode:baseLayout.message.agentAvtarUrl];
...
//    self.loadingView.right = self.bubbleImageView.left-kUAVSCellBubbleToIndicatorSpacing;
//    self.loadingView.size = CGSizeMake(kUAVSCellLoadingSize, kUAVSCellLoadingSize);
...
//
//            [self setupMsgStatusLabelWithContent:getUDAVSLocalizedString(@"udesk_msg_revoke")];
//            break;
//        case UDMessageSendStatusOffSending:
//
//            [self setupMsgStatusLabelWithContent:getUDAVSLocalizedString(@"udesk_msg_sendOffline")];
//            break;
//
//        default:
//            break;
...
//    //消息状态
//    self.msgStatusLabel.top = self.bubbleImageView.bottom+kUAVSChatMessageStatusToVerticalEdgeSpacing;
//    self.msgStatusLabel.size = CGSizeMake(width, kUAVSChatMessageStatusHeight);
...
- (void)showMenuControllerInView:(UIView *)inView
    ...
    if (self.baseLayout.message.messageType == UdeskAVSMessageContentTypeText) {
        UIMenuItem *copyTextItem = [[UIMenuItem alloc] initWithTitle:getUDAVSLocalizedString(@"uavs_common_copy") action:@selector(copyTextSender:)];
...
//        self.baseLayout.message.sendStatus != UDMessageSendStatusRollback &&
        ...
        UIMenuItem *rollbackItem = [[UIMenuItem alloc] initWithTitle:getUDAVSLocalizedString(@"uavs_common_revoke") action:@selector(rollbackSender:)];
...
- (void)rollbackSender:(id)sender {
    ...
    NSTimeInterval timeInterval = self.baseLayout.message.timestamp;
    if (timeInterval > 120) {
        UIAlertController *alert = [UIAlertController alertControllerWithTitle:nil message:getUDAVSLocalizedString(@"uavs_tip_revoke_timeOut") preferredStyle:UIAlertControllerStyleAlert];
        [alert addAction:[UIAlertAction actionWithTitle:getUDAVSLocalizedString(@"uavs_common_makeSure") style:UIAlertActionStyleDefault handler:nil]];
