...
@interface UAVSChatTextLinePositionModifier()<YYTextLinePositionModifier>

@end


@implementation UAVSChatTextLinePositionModifier
...
- (id)copyWithZone:(NSZone *)zone {
    UAVSChatTextLinePositionModifier *one = [self.class new];
    one->_font = _font;
...
@implementation UAVSChatTextLayout
...
- (instancetype)initWithMessage:(UdeskAVSBaseMessage *)message dateDisplay:(BOOL)dateDisplay {
    ...
    if (self) {
        ...
        self.height += kUAVSCellBubbleToTextVerticalSpacing*2;
...
- (void)_layoutText {
    ...
    NSMutableAttributedString *text = [self _textWithMessage:self.message
                                                   isRetweet:NO
                                                    fontSize:kUAVSCellTextFontSize
                                                   textColor:[UIColor colorWithRed:0.165f  green:0.165f  blue:0.165f alpha:1]];
    if (text.length == 0 || !text) return;
    
    UAVSChatTextLinePositionModifier *modifier = [UAVSChatTextLinePositionModifier new];
    modifier.font = [UIFont fontWithName:kUAVSTextFontName size:kUAVSCellTextFontSize];
    
    CGFloat contentWidth = kScreenWidth > 320 ? kUAVSCellTextContentWidth : kUAVS320CellTextContentWidth;
    ...
    for (NSString *numberRegex in numberRegexs) {
        ...
        if (range.location != NSNotFound) {
            ...
            if ([text yy_attribute:YYTextHighlightAttributeName atIndex:range.location] == nil) {
                ...
                // 数据信息，用于稍后用户点击
                highlight.userInfo = @{kUAVSNumberName : [message.content substringWithRange:range]};
    ...
    for (NSString *linkRegex in linkRegexs) {
        ...
        if (range.location != NSNotFound) {
            ...
            if ([text yy_attribute:YYTextHighlightAttributeName atIndex:range.location] == nil) {
                ...
                [highlight setBackgroundBorder:highlightBorder];
                // 数据信息，用于稍后用户点击
                highlight.userInfo = @{kUAVSLinkURLName : [message.content substringWithRange:range]};
                [text yy_setTextHighlight:highlight range:range];
            }
        }
    }
...
- (void)_layoutBubbleSize {
    ...
    CGFloat textBubbleWidth = self.textLayout.textBoundingSize.width+(kUAVSCellBubbleToTextHorizontalSpacing*2);
    CGFloat textBubbleHeight = self.textLayout.textBoundingSize.height+(kUAVSCellBubbleToTextVerticalSpacing*2);
...
- (UITableViewCell *)getCellWithReuseIdentifier:(NSString *)cellReuseIdentifer {
    ...
    return [[UAVSChatTextCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellReuseIdentifer];
}
