 ...
 */
static NSArray *_UavsNSBundlePreferredScales() {
 ...
 */
static NSString *_UavsNSStringByAppendingNameScale(NSString *string, CGFloat scale) {
...
@implementation UAVS_YYImage {
    UAVS_YYImageDecoder *_decoder;
    NSArray *_preloadedFrames;
    dispatch_semaphore_t _preloadedLock;
    NSUInteger _bytesPerFrame;
}

+ (UAVS_YYImage *)imageNamed:(NSString *)name {
    ...
    NSArray *scales = _UavsNSBundlePreferredScales();
    for (int s = 0; s < scales.count; s++) {
        scale = ((NSNumber *)scales[s]).floatValue;
        NSString *scaledName = _UavsNSStringByAppendingNameScale(res, scale);
...
+ (UAVS_YYImage *)imageWithContentsOfFile:(NSString *)path {
    return [[self alloc] initWithContentsOfFile:path];
}

+ (UAVS_YYImage *)imageWithData:(NSData *)data {
    return [[self alloc] initWithData:data];
}

+ (UAVS_YYImage *)imageWithData:(NSData *)data scale:(CGFloat)scale {
    return [[self alloc] initWithData:data scale:scale];
}

- (instancetype)initWithContentsOfFile:(NSString *)path {
    NSData *data = [NSData dataWithContentsOfFile:path];
    return [self initWithData:data scale:_UavsNSStringPathScale(path)];
}
...
- (instancetype)initWithData:(NSData *)data scale:(CGFloat)scale {
    ...
    @autoreleasepool {
        ...
        UAVS_YYImageFrame *frame = [decoder frameAtIndex:0 decodeForDisplay:YES];
