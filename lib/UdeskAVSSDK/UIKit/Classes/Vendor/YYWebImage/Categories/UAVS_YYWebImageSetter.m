...
NSString *const UAVS_YYWebImageFadeAnimationKey = @"YYWebImageFade";
const NSTimeInterval UAVS_YYWebImageFadeTime = 0.2;
const NSTimeInterval UAVS_YYWebImageProgressiveFadeTime = 0.4;


@implementation UAVS_YYWebImageSetter {
...
- (int32_t)setOperationWithSentinel:(int32_t)sentinel
    ...
    if (sentinel != _sentinel) {
        if (completion) completion(nil, imageURL, UAVS_YYWebImageFromNone, UAVS_YYWebImageStageCancelled, nil);
        return _sentinel;
    }
    ...
    if (!operation && completion) {
        ...
        completion(nil, imageURL, UAVS_YYWebImageFromNone, UAVS_YYWebImageStageFinished, [NSError errorWithDomain:@"com.ibireme.webimage" code:-1 userInfo:userInfo]);
    }
