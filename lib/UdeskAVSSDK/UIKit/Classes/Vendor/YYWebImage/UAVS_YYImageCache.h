...
@class UAVS_YYMemoryCache, UAVS_YYDiskCache;

NS_ASSUME_NONNULL_BEGIN

/// Image cache type
typedef NS_OPTIONS(NSUInteger, UAVS_YYImageCacheType) {
    /// No value.
    UAVS_YYImageCacheTypeNone   = 0,
    
    /// Get/store image with memory cache.
    UAVS_YYImageCacheTypeMemory = 1 << 0,
    
    /// Get/store image with disk cache.
    UAVS_YYImageCacheTypeDisk   = 1 << 1,
    
    /// Get/store image with both memory cache and disk cache.
    UAVS_YYImageCacheTypeAll    = UAVS_YYImageCacheTypeMemory | UAVS_YYImageCacheTypeDisk,
};
 ...
 */
@interface UAVS_YYImageCache : NSObject
...
@property (strong, readonly) UAVS_YYMemoryCache *memoryCache;

/** The underlying disk cache. see `YYDiskCache` for more information.*/
@property (strong, readonly) UAVS_YYDiskCache *diskCache;
...
- (void)setImage:(nullable UIImage *)image
       imageData:(nullable NSData *)imageData
          forKey:(NSString *)key
        withType:(UAVS_YYImageCacheType)type;
...
/**
 ...
 @return The image associated with key, or nil if no image is associated with key.
 */
- (nullable UIImage *)getImageForKey:(NSString *)key withType:(UAVS_YYImageCacheType)type;
...
- (void)getImageForKey:(NSString *)key
              withType:(UAVS_YYImageCacheType)type
             withBlock:(void(^)(UIImage * _Nullable image, UAVS_YYImageCacheType type))block;
