...
+ (void)storeGroupId:(NSString *)groupId {
    @try {
        //用户传入GroupId
        if ([UdeskAVCSUtil isBlankString:groupId]) {
...
+ (void)storeMenuId:(NSString *)menuId {
    @try {
        //用户传入GroupId
        if ([UdeskAVCSUtil isBlankString:menuId]) {
...
+ (NSRange)linkRegexsMatch:(NSString *)content {
    ...
    NSArray *numberRegexs = [UdeskAVCSUtil linkRegexs];
    // 数字正则匹配
...
+ (NSString *)urlEncode:(NSString *)url {
    if ([UdeskAVCSUtil isBlankString:url]) return url;
    ...
    if ([url rangeOfString:@"?"].location != NSNotFound) {
        ...
        for (NSString *paramsStr in paramsArray) {
            ...
            NSString *key = [UdeskAVCSUtil percentEscapedStringFromString:keyValues.firstObject];
            NSString *value = [UdeskAVCSUtil percentEscapedStringFromString:keyValues.lastObject];
